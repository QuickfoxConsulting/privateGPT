...
from llama_index.core.node_parser import SemanticSplitterNodeParser, SentenceSplitter, SentenceWindowNodeParser, MarkdownNodeParser
...
class IngestService:
    ...
    def __init__(
        ...
        # Select the appropriate node parser based on settings
        ingest_mode = settings().embedding.ingest_mode
        transformations: list[TransformComponent] = []
        
        if ingest_mode == "hierarchical":
            # ParentDocumentIngestComponent manages its own HierarchicalNodeParser
            node_parser = None
        else:
            # All other modes (simple, batch, parallel, pipeline) use LateChunkNodeParser
        ...
        # Add summary extractor if enabled (currently commented out)
        # transformations.append(SummaryExtractor(llm=self.llm_service.llm, summaries=["prev", "self"]))
        
        # Always add the embedding model as the final transformation
        transformations.append(embedding_component.embedding_model)
    ...
    async def ingest_url(self, url: str, metadata: dict[str, str] | None = None) -> list[IngestedDoc]:
        logger.debug("Ingesting url=%s", url)
        documents = await self.ingest_component.ingest(url=url, metadata=metadata)  
        return [IngestedDoc.from_document(document) for document in documents]
