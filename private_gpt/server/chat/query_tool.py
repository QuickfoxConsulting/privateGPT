...
Enhanced query:"""
    ...
    def call(self, query: str) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                ...
                raw_input={"query": query, "enhanced_query": enhanced_query},
                raw_output=response,
            )
            
        except Exception as e:
            logger.error(f"Enhanced retrieval failed for query '{query}': {e}")
            return ToolOutput(
    ...
    async def acall(self, query: str) -> ToolOutput:
        ...
        try:
            ...
            if self._enable_query_expansion and len(query.split()) > 2:
                enhancement_prompt = f"""Given this search query, enhance it to improve document retrieval.
...
Enhanced query:"""
            ...
            logger.error(f"Async enhanced retrieval failed for query '{query}': {e}")
            return ToolOutput(
                content=f"I encountered an error while searching the documents: {str(e)}",
                tool_name=self.metadata.name,
                raw_input={"query": query},
