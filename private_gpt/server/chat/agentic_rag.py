...
class AgenticCondenseChatEngine(BaseChatEngine):
    ...
    def from_defaults(
        ...
        **kwargs: Any,
        ...
        memory = memory or ChatMemoryBuffer.from_defaults(
            chat_history=chat_history, token_limit=llm.metadata.context_window * 0.8
        )
    ...
    def _run_agentic_condense_sync(
        ...
        # 1. Memory Setup (History Restoration)
        if chat_history is not None:
            self._memory.set(chat_history)
        
        # 2. Condense - CONVERT TO STANDALONE QUERY FIRST
        # This allows follow-up queries like "summarize that document" to work
        current_history = self._memory.get(input=message)
        standalone_question = self._condense_question(current_history, message)
        
        # 3. Tool Routing Check (on Standalone Question)
        tool_intent = self._detect_specific_document_intent(standalone_question)
        tool_output = None
        if tool_intent:
            logger.info(f"Detected tool intent for condensed query: {tool_intent}")
            tool_output = self._create_and_run_tool(tool_intent, standalone_question)
        ...
        # 4 & 5. Retrieve (Standard or Tool-based)
        if tool_output:
        ...
        # 7. Prepare Final History (with Token-Budget protection)
        # We cap the context tokens impact on history pruning to ensure history is used
        # We reserve at least 30% of the context window for history
        context_token_estimate = self._token_counter.estimate_tokens(context_str)
        system_token_estimate = self._token_counter.estimate_tokens(final_system_msg)
        
        history_reservation = int(self._llm.metadata.context_window * 0.3)
        initial_token_count = min(
            system_token_estimate + context_token_estimate,
            self._llm.metadata.context_window - history_reservation
        )
    ...
    async def _arun_agentic_condense(
        ...
        # 3. Tool Routing Check (Async)
        tool_intent = self._detect_specific_document_intent(standalone_question)
        tool_output = None
        if tool_intent:
            logger.info(f"Detected tool intent (Async) for condensed query: {tool_intent}")
            try:
                # Run blocking tool in thread pool to avoid blocking event loop
                tool_output = await asyncio.to_thread(
                    self._create_and_run_tool, tool_intent, standalone_question
                )
                if tool_output.is_error:
                     logger.warning(f"Tool failed: {tool_output.raw_output}. Falling back to standard RAG.")
                     tool_output = None
