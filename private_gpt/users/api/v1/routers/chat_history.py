...
def read_chat_history(
    ...
    try:
        chat_history_orm = crud.chat.get_by_id(db, id=conversation_id)

        if chat_history_orm is None or chat_history_orm.user_id != current_user.id:
            raise HTTPException(
        ...
        response_data = schemas.ChatHistory.model_validate(chat_history_orm)
        response_data.chat_items = [schemas.ChatItem.model_validate(item) for item in paginated_chat_items_orm]
